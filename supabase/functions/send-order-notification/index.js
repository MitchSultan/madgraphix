// Follow this Deno runtime for Supabase Edge Functions
// https://deno.land/
...
serve(async (req) => {
  try {
    ...
    // Prepare email data
    const adminEmail = getAdminOrderNotificationEmail(order)
    const clientEmail = getClientConfirmationEmail(order)
    ...
    const emailPromises = [
      ...
      fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'MAD Graphix Orders <orders@madgraphix.co.ke>',
          to: [ADMIN_EMAIL],
      ...
      fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'MAD Graphix <noreply@madgraphix.co.ke>',
...
function getAdminOrderNotificationEmail(order) {
...
function getClientConfirmationEmail(order) {
  const itemsList = order.order_items
    .map(item => `
